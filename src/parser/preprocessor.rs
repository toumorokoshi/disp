...
/*
pub fn parse_block(indent: usize, input: &mut Peekable<Chars>) -> String {
    let mut buffer = String::from("[");
    let mut has_more = None != input.peek();
    while has_more {
        buffer.push_str(&parse_statement(input));
        has_more = None != input.peek();
    }
    buffer.push(']');
    return buffer;
}

pub fn parse_statement(input: &mut Peekable<Chars>) -> String {
    let mut buffer = String::new();
    let wrap_with_parens = match input.peek() {
        Some(c) => c != &'(',
        None => false
    };
    if wrap_with_parens {
